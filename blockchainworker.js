  ...
  async fetch(request, env, ctx) {
    ...
    if (!globalThis.dbInitialized) {
      try {
        ...
        const seedNode = 'https://black-violet-065c.a624587332.workers.dev';
        await env.DB.prepare(`
          INSERT OR IGNORE INTO blockchain_peers (domain, is_beacon, last_seen, reputation_score) 
          VALUES (?, 'true', ?, 999)
        `).bind(seedNode, Date.now()).run();
    ...
    if (url.pathname.startsWith('/api/consensus/')) {
        ...
        if (request.method === 'POST' && route === 'register') {
            try {
                ...
                if (data.domain && data.is_beacon) {
                    await env.DB.prepare(`
                        INSERT INTO blockchain_peers (domain, is_beacon, vps_count, total_asset, last_seen) 
                        VALUES (?, ?, ?, ?, ?) 
                        ON CONFLICT(domain) DO UPDATE SET is_beacon=excluded.is_beacon, last_seen=excluded.last_seen
                    `).bind(data.domain, data.is_beacon, data.vps_count || 0, data.total_asset || 0, Date.now()).run();
                }
        ...
        if (request.method === 'GET' && route === 'sync') {
            ...
            const { results: peers } = await env.DB.prepare('SELECT * FROM blockchain_peers WHERE is_beacon = "true" ORDER BY reputation_score DESC LIMIT 10').all();
            return new Response(JSON.stringify({ blocks, peers }), { headers: {'Content-Type':'application/json'} });
        }

        if (request.method === 'POST' && route === 'submit') {
            if (sys.is_beacon !== 'true') return new Response('Not a beacon', { status: 403 });
            try {
                ...
                if (currentBlock) {
                    ...
                    if (incomingPayload.total_asset <= currentPayload.total_asset) {
                        shouldInsert = false; 
                    }
                }
                ...
                if (shouldInsert) {
                    ...
                    `).bind(block.proposer_domain, pl.vps_count, pl.total_asset, Date.now()).run();
                }
                return new Response('Consensus Accepted', { status: 200 });
    ...
    const mineAndGossip = async (localAsset, localVpsCount) => {
        try {
            ...
            if (Math.random() < 0.1) {
                const { results: beacons } = await env.DB.prepare(`SELECT domain FROM blockchain_peers WHERE is_beacon = 'true' ORDER BY RANDOM() LIMIT 1`).all();
                if (beacons.length > 0) {
                    ...
                    if(syncRes.ok) {
                        ...
                        for(const b of syncData.blocks) {
                            ...
                            await env.DB.prepare(`INSERT INTO blockchain_peers (domain, vps_count, total_asset, last_seen) VALUES (?, ?, ?, ?) ON CONFLICT(domain) DO UPDATE SET vps_count=excluded.vps_count, total_asset=excluded.total_asset, last_seen=excluded.last_seen`).bind(b.proposer_domain, pl.vps_count, pl.total_asset, b.timestamp).run();
                        }
                    }
                }
            }
...
`;
    ...
    if (request.method === 'GET' && url.pathname === '/') {
      ...
      try {
          ...
          const { results: beacons } = await env.DB.prepare('SELECT count(*) as c FROM blockchain_peers WHERE is_beacon="true"').first();
